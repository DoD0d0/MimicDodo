...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    # --- FIX 3: MOTION FILE LADEN ---
    # Hier entscheiden wir: Lokal oder Cloud?
    if args_cli.motion_file:
        ...
        # (Original Logik für WandB Path - lassen wir so)
        import wandb
        ...
        # Fallback auf lokalen Ordner + Registry Download
        print(f"[INFO] Loading experiment from directory: {log_root_path}")
        resume_path = get_checkpoint_path(log_root_path, agent_cfg.load_run, agent_cfg.load_checkpoint)
        print(f"[INFO]: Loading model checkpoint from: {resume_path}")
        
        # Falls Registry Name angegeben wurde
        if args_cli.registry_name:
            print(f"[INFO] ☁️ Downloading from WandB: {args_cli.registry_name}")
            import wandb
            api = wandb.Api()
            reg_name = args_cli.registry_name
            if ":" not in reg_name: reg_name += ":latest"
            artifact = api.artifact(reg_name)
            motion_path = artifact.download()
            env_cfg.commands.motion.motion_file = str(pathlib.Path(motion_path) / "motion.npz")
    # --------------------------------
